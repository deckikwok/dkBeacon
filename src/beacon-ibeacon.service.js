...
export default class dkbeaconIbeaconService {
    ...
    constructor(webDebug) {
        ...
        self._iBeaconsObj = {};
        self._iBeaconsSubject = new BehaviorSubject([]);
        self._iBeaconScanStatusSubject = new BehaviorSubject(false);
    ...
    simulateIBeacons() {
        ...
        setInterval(() => {
            let newFauxIBeacon = new BeaconClass.IBeaconDeviceClass("47:80:8D:F7:02:6" + counterId.toString(), 'ibeaconDecki', 'https://www.apple.com', -88, 'IBeacon');
            self._iBeaconsObj[newFauxIBeacon.id] = newFauxIBeacon;
            let fauxIBeaconsArr = Object.keys(self._iBeaconsObj).map((key) => {
...
if (this._webDebug === false) {
     ...
     */
    delegate.monitoringDidFailForRegionWithError().subscribe((data) => {
     ...
     */
    delegate.peripheralManagerDidStartAdvertising().subscribe((data) => {
     ...
     */
    delegate.didChangeAuthorizationStatus().subscribe((data) => {
