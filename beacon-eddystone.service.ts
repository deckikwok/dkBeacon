...
import {BeaconTelemetryClass} from '../../app-tsclasses/beacon-telemetry.class'
import {BehaviorSubject, Observable} from "rxjs";
import {buildTarget} from "../app-environment.constants";
import {BeaconClass} from "./class/beacon-device.class";
import {BLE} from 'ionic-native';

let self: any;

@Injectable()
export class BeaconEddystoneService {

  private _eddyBeaconsObj: Object;
  private _eddyBeaconsSubject: BehaviorSubject<Array<any>>; //fixme: type
  private _isScanning: boolean;
  private _eddyScanStatusSubject: BehaviorSubject<boolean>;
  ...
  constructor() {
    ...
    self._eddyBeaconsSubject = <BehaviorSubject<Array<any>>> new BehaviorSubject([]);
    self._eddyScanStatusSubject = <BehaviorSubject<boolean>> new BehaviorSubject(false);
  ...
  onDeviceReady() {
    ...
    if (buildTarget !== 'cordova') {
      console.log('not cordova. returning...')
      return
    }
  ...
  scanEddystoneBeacons() {
    ...
    BLE.startScanWithOptions(
      ['0000FEAA-0000-1000-8000-00805F9B34FB'], //'0000FEAA-0000-1000-8000-00805F9B34FB'
      ...
      (device: any) => {
        ...
        let eddyBeaconsArr = Object.keys(self._eddyBeaconsObj).map(
          (key) => {
...
//ON WEB FOR MOCK RESULTS ========================================================================
  ...
  simulateEddystoneBeacons() {
    ...
    if (buildTarget !== 'web') {
      console.log('not web. returning from simulate eddy beacons...')
      return
    }
    ...
    setInterval(() => {
      let newFauxEddyBeacon = new BeaconClass.EddystoneDeviceClass(
      ...
      //fixme MAP TO ARRAY
      let fauxEddyBeaconsArr = Object.keys(self._eddyBeaconsObj).map(
        (key) => {
          return self._eddyBeaconsObj[key]
        }
      )
...
function parseEddystoneFrame(device: any, data: Uint8Array) { //fixme: typescript
...
function parseEddystoneURL(data: Uint8Array) {
  let url = '';
...
let messageLog: {messageText: string}[] = [];
...
function mapBeaconRSSI(rssi: number) {
